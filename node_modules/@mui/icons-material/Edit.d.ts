...
const ProductCard = ({ product, itIsOff }) => {
    const { addProductCart, removeProduct, getProductCart } = useContext(ShoppingCartContext);
...
state = {{ product }}>
    <EditIcon/>
    < /IconButton>
    < IconButton onClick = { handleRemoveProduct } > <DeleteIcon/></IconButton >
        </Box>
        < /Box>
        < CardMedia
component = "img"
...
{ product.isPromotion && <p><span>Precio promocional: </span> {`${product.price - (product.price / 100 * itIsOff ) } `}</p>}
            ...
            <CardActions className="product-card__actions">
                ...
                <span>{quantity}</span>
                <Button onClick={handleAddProduct}><AddIcon/></Button>
            </CardActions>
        </Card>
    );
};
